...
module ChessHelper
   'a'.upto('h').each_with_index do | file, f |
     8.downto(1).each_with_index do | rank, r |
 ...
 def self.load(file_name)
  ...
  def move_king(from, to)
    ...
    if 1 < [dx.abs, dy.abs].max then
      if from == white(piece, e1, e8) && to == white(piece, g1, g8) && castling.include?(white(piece,"K","k"))
        ...
        move_piece(white(a1, a8), white(piece, d1, d8))
      else
        return false
      end
    end
  ...
  def move_pawn(from, to)
    ...
    if to == ep && dx.abs == 1 && dy == white(piece,-1,1) then #en passant
      ...
      return (board[to] == "-" && dx == 0 && dy == white(piece,-1,1) || # moving 1 square
              board[to] == "-" && dx == 0 && dy == white(piece,-2,2) && from/8 == white(piece,6,1) ||
  ...
  def move_piece(from, to)
    ...
    board[to] = board[from]
    board[from] = "-"
    @ep = piece.upcase == "P" && dy.abs == 2 ? to + white(piece,8,-8)
    valid || in_check? ? self : nil
  end
...
class AmbiguousMove < Exception
  ...
  def move(str)
    ...
    if m = str.match(/^ (?<piece>[RNBQK])?  (?<file>[a-h])?(?<rank>[1-8])  x?  (?<sq>[a-h][1-8])  (=(?<promote>[RNBQ]))? \+? $
       ...
       is_piece_taken = board[0] != "-" || piece.upcase == "P" && to == ep
       raise IllegalMove.new(str,self) if color(piece) != turn

       list = INDICES.select do |from|
         tmp = postion.dup
       ...
       position.board[to] = m[:promote].send(white(piece, :upcase, :downcase))  if promote && piece.upcase == "P"
       position.turn = white(postion.turn, :black, :white)
